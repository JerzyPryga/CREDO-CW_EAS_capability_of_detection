...
string get_current_dir();							//Returns path to current directory

//-------------------------------------------------------------

/* 	The majority of the code is
	a ROOT macro called "Rho_rNE". All computations
...
void Rho_rNE()
{
  ...
  */
  ...
  cout<< "(default: the last file read from the dictionary) :";					//Inputing plots_dir
  ...
  */
  ...
  for(int rp = 0; rp < r_number; rp++) {
    ...
    if(rp == (r_number - 1)) R_fit_lst[rp] = R_fit_lst[rp] - 100.;		//Modification preventing from getting out of histogram range
  ...
  */
  ...
  else cout<< "ERROR: COULD NOT OPEN THE SIMULATIONS DIRECTORY" <<endl;
  ...
  */
  ...
  TH1 * h_Rho_avr = new TH1F("h_Rho_avr", Rho_avr_title , bins_rho_r - 1, 0, r_prc_max);
  ...
  */
  ...
  cout<<endl;
  h_Rho_avr->Draw();
  A.SaveAs(Rho_avr_path);							//Saving it in the created directory
  ...
  */
  ...
  string path_Rho_rN_1D = Rho_rN_1D_norm_E;
  path_Rho_rN_1D.append("/Rho_(r,N)_1D_norm_");
  path_Rho_rN_1D.append(to_string(R_fit_lst[rf]));
  path_Rho_rN_1D.append(".png");

  const char* Rho_rN_1D_path = path_Rho_rN_1D.c_str();

  cout<<endl;
  g_Rho_rN_1D->Draw("AP");
  L.SaveAs(Rho_rN_1D_path);
  g_Rho_rN_1D->Delete();
  ...
  */
  ...
  string path_Rho_E_r = Rho_rE_norm_dir;
  path_Rho_E_r.append("/Rho_(E)_norm_");
  path_Rho_E_r.append(to_string(R_fit_lst[rf]));
  path_Rho_E_r.append(".png");

  const char* Rho_E_r_path = path_Rho_E_r.c_str();

  cout<<endl;
  g_Rho_E->Draw("AP");
  N.SaveAs(Rho_E_r_path);
  g_Rho_E->Delete();
