...
int symbol_newtona(int a, int b);
long double Qnk(int n, int k, long double P);
long double FjE(long double Emin, long double Emax_temp);
long double Fluct_rho(long double rho, int k);
long double Qnk_new(int n, int k, double Adet, long double rho);
long double array_avr(double array[], int n);
...

{
  ...
  */
  ...
  double Rhor_params[6];
  double RhoN_param0[our_l], RhoN_param1[our_l];  
  double RhoE_param0[our_l], RhoE_param1[our_l], RhoE_param2[our_l];  
  ...
  string Rhor_param_file_name = params_dir;
  Rhor_param_file_name.append("/Rho(r)_fit_params.txt");

  ifstream Rhor_param_fit_file(Rhor_param_file_name);
  string Rhor_line_fit;

  int ipar_Rhor = 0;

  if(Rhor_param_fit_file.is_open()) {

    while(getline(Rhor_param_fit_file, Rhor_line_fit)) {		//Parameters of Rho(r) function
      Rhor_params[ipar_Rhor] = stod(Rhor_line_fit);
      //cout << Rhor_params[ipar_Rhor] <<endl;
      ipar_Rhor++;
    }

  }

  else cout<< "ERROR: COULD NOT FIND THE Rho(R) FIT PARAMETERS FILE" <<endl;

  Rhor_param_fit_file.close();
  ...
  double RhoN_p0 = array_avr(RhoN_param0, our_l);
  double RhoN_p1 = array_avr(RhoN_param1, our_l);

  //-------------------------

  string RhoE_param_file_name = params_dir;
  RhoE_param_file_name.append("/Rho(r,E)_fit_params.txt");

  ifstream RhoE_param_fit_file(RhoE_param_file_name);
  string RhoE_line_fit;
  ...
  if(RhoE_param_fit_file.is_open()) {

    while(getline(RhoE_param_fit_file, RhoE_line_fit)) {
     									//Parameter [0] of Rho(E/Enorm) function
      RhoE_param0[iparE] = stod(RhoE_line_fit);
      //cout << RhoE_param0[iparE] << " : ";
      getline(RhoE_param_fit_file, RhoE_line_fit);			//Parameter [1] of Rho(E/Enorm) function
      RhoE_param1[iparE] = stod(RhoE_line_fit);
      //cout << RhoE_param1[iparE] << " : ";
      getline(RhoE_param_fit_file, RhoE_line_fit);			//Parameter [2] of Rho(E/Enorm) function
      RhoE_param2[iparE] = stod(RhoE_line_fit);
      //cout << RhoE_param2[iparE] <<endl;
      iparE++;
    }

  }

  else cout<< "ERROR: COULD NOT FIND THE Rho(E/Enorm) FIT PARAMETERS FILE" <<endl;

  RhoE_param_fit_file.close();

  double RhoE_p0 = array_avr(RhoE_param0, our_l);
  //cout << RhoE_p0 << " : ";
  double RhoE_p1 = array_avr(RhoE_param1, our_l);
  //cout << RhoE_p1 << " : ";
  double RhoE_p2 = array_avr(RhoE_param2, our_l);
  //cout << RhoE_p2 <<endl;
  ...
  */
  ...
  /* 	This part creates Rho(r, E) function.
  */

  TF1 F_E("F_E", " [0]*pow(x, [1]) + [2]");					//Density factor for energy (F_E)
  F_E.SetParameters(RhoE_p0, RhoE_p1, RhoE_p2);

  //-----------------------------------------------------------------

  TF1 F("F"," [0]*pow((x + [1]), -[2]) + [3]*exp(-[4]*x) + [5] ");  					//Particles density function
  TF1 Fg("Fg"," (2*[6]) * x * ( [0]*pow((x + [1]), -[2]) + [3]*exp(-[4]*x) + [5]) ");  			//Function for integration

  F.SetParameters(Rhor_params[0], Rhor_params[1], Rhor_params[2], Rhor_params[3], Rhor_params[4], Rhor_params[5]);
  Fg.SetParameters(Rhor_params[0], Rhor_params[1], Rhor_params[2], Rhor_params[3], Rhor_params[4], Rhor_params[5], M_PI);
  ...
  */
  ...
  cout<<"Number of particles for found density function			N_part = " << F_E(E_e) * Npart_max <<endl;
  ...
  */  
  ...
  for(int rka = 0; rka < eps; rka++) {					//Start DISTANCE loop
    ...
    for(int phi_c = 0; phi_c < eps_phi; phi_c++) {			//Start ANGLE loop
      ...
      long double F_fluct_czyn_temp = (gen.Gaus(f_meanNE->Eval(E_e), f_sigmaNE->Eval(E_e))) / f_meanNE->Eval(E_e);
      if(F_fluct_czyn_temp < 0.0) F_fluct_czyn_temp = 1.0;
      long double F_fluct_czyn = F_fluct_N(F_fluct_czyn_temp);

      long double rho_temp = 0;
      if(F_theta(angle) >= 0.0) rho_temp = F(Rcascn[rka]) * F_theta_czyn * F_E_czyn * F_fluct_czyn;
      ...
      for(int k_temp = n; k_temp > 0; k_temp--) {			//Start K loop
        ...
        //Qcasc[k_temp][rka][phi_c][f] = Qnk(n, k_temp, PcascNR[rka][phi_c][f]) * Qnk(n - k_temp, 0, Pbg);

        //--------------------------------------------------		//No coincidance with background - new method

        Qcasc[k_temp][rka][phi_c][f] = Qnk_new(n, k_temp, Adet_phi[phi_c], rho_temp) * Qnk(n - k_temp, 0, Pbg);
...
long double Qnk(int n, int k, long double P)
{
   return (symbol_newtona(n, k) * pow(P, k) * pow((1 - P), (n - k)));
}
...
long double Qnk_new(int n, int k, double Adet, long double rho)
{
  ...
  long double Qtemp = 1.0;
  long double Q_n_temp = 1.0;
  long double Qnk_result;

  for(int k_n = 0; k_n <= k; k_n++) {

    long double P_temp = Adet * Fluct_rho(rho, k_n) * rho;
    P_temp = 1 - exp( - P_temp);

    if(k_n > 0) Qtemp = Qtemp * P_temp;

  }

  for(int k_n = 0; k_n <= (n - k); k_n++) {

    long double P_temp = Adet * Fluct_rho(rho, k_n) * rho;
    P_temp = 1 - exp( - P_temp);

    if(k_n > 0) Q_n_temp = Q_n_temp * (1 - P_temp);

  }

  Qnk_result = symbol_newtona(n, k) * Qtemp * Q_n_temp;
  if(Qnk_result >= 1.0) Qnk_result = 1.0;
...
long double FjE(long double Emin, long double Emax)
{
...
long double Fluct_rho(long double rho, int k)
{
  ...
  if(k > 1) {
   if(rho < 2*pow(10, -2)) return (1.05 * pow(rho, 0));
   if( (rho >= 2*pow(10, -2)) && (rho < pow(10, 0)) ) return (5.73886 * pow(rho, 0.12589));
   if( (rho >= pow(10, -8)) && (rho < 7*pow(10, 0)) ) return (0.174958 * pow(rho, -0.25308));
   if(rho >= 7*pow(10, 0)) return (1.05 * pow(rho, 0));
  }
